...
function getStorageKey(d) { return `eczema_day_${d}`; }

function loadDay(day) {
  try {
    const raw = window.localStorage?.getItem(getStorageKey(day));
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
}

function saveDay(day, data) {
  try { window.localStorage?.setItem(getStorageKey(day), JSON.stringify(data)); } catch {}
}

function loadStartDate() {
  try { return window.localStorage?.getItem("eczema_start_date") || null; } catch { return null; }
}

function saveStartDate(d) {
  try { window.localStorage?.setItem("eczema_start_date", d); } catch {}
}
...
export default function EczemaTracker() {
  ...
  const getCompletionCount = () => {
    let count = 0;
    for (let i = 1; i <= 14; i++) { if (loadDay(i)?.timestamp) count++; }
    return count;
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          {Array.from({ length: 14 }, (_, i) => {
            const d = i + 1;
            const hasEntry = loadDay(d)?.timestamp;
